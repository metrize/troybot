...
$(window).on('load', (function () {
    ...
    $('.volume-container').each(function (index, element) {
        ...
        let playerWrapper = $(element).closest('tr').find('.play-in-browser-wrapper');
    ...
    $('.link-input').on('input propertychange paste', function () {
        if (!this.value.match(/^https:\/\/\S*$/)) {
            this.setCustomValidity("Playsound links must start with https:// and may not contain spaces");
            return;
        } else {
        ...
        $(this).closest('tr').find('.play-in-browser-wrapper').attr('data-link', newValue);
    });
    ...
    $('.playsound-form').each(function (index, form) {
        ...
        // the DELETE button
        let deleteButton = $(`button.playsound-submit-delete[form=\"${formID}\"]`);
        ...
        $(form).closest('td').find('.play-in-browser-wrapper').each(function (index, wrapper) {
            ...
            $(playButton).click(() => {
                ...
                doSave(() => {
                    $.ajax({
                        url: `/api/v1/playsound/${encodeURIComponent(name)}/play`,
                        type: 'POST'
                    })
                });
            })
