...
$(document).mouseup(function(e) {
    if (temp_volume != player.volume) {
        $('.plyr__volume').find('input')[0].style.cssText =
            '--value:' + player.volume * 100 + '%';
        $('.plyr__volume').find('input')[0].value = player.volume;
        socket.send(
            JSON.stringify({ event: 'VOLUME', data: { volume: temp_volume } })
        );
    }
    if (seek != -1) {
        console.log((seek / 100) * player.media.duration);
        $('.plyr__progress').find('input')[0].value =
            (player.currentTime / player.media.duration) * 100;
        $('.plyr__progress').find('input')[0].style.cssText =
            '--value:' +
            (player.currentTime / player.media.duration) * 100 +
            '%';
        socket.send(
            JSON.stringify({
                event: 'SEEK',
                data: { seek_time: (seek / 100) * player.media.duration },
            })
        );
        seek = -1;
    }
        ...
        player = new Plyr('#audio1', {
            ...
            listeners: {
                play: function(e) {
                    ...
                    if (paused) {
                        socket.send(
                            JSON.stringify({ event: 'RESUME', data: {} })
                        );
                    } else {
                        socket.send(
                            JSON.stringify({ event: 'PAUSE', data: {} })
                        );
                    }
                    //
...
function initialize(data) {
    ...
    if (data.currentSong != null) {
        play({
            video_id: data['currentSong']['video_id'],
            video_title: data['currentSong']['video_title'],
            requested_by: data['currentSong']['requested_by'],
        });
...
function update_playlist({ playlist }) {
    ...
    for (var i = 0; i < playlist.length; i++) {
        ...
        var trackNumber = i + 1,
            ...
            video_length =
                Math.floor(video_length / 60).toString() +
                ':' +
                (video_length - Math.floor(video_length / 60) * 60)
                    .toString()
                    .padStart(2, '0');
    ...
    $('.plRemove').click(function(event) {
        ...
        socket.send(
            JSON.stringify({
                event: 'REMOVE',
                data: { database_id: tracks[id].database_id },
            })
        );
    });
...
function update_history({ history }) {
    ...
    $('.buRequeue').click(function(event) {
        ...
        console.log({
            event: 'REQUEUE',
            data: { database_id: hist_tracks[id].database_id },
        });
        socket.send(
            JSON.stringify({
                event: 'REQUEUE',
