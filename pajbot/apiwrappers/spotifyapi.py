...
class SpotifyApi(BaseAPI):
    ...
    def pause(self, token_manager):
        ...
        if not self.device_id:
            return False
        log.info(self.put(endpoint="me/player", headers=headers, json={"device_ids":[f"{self.device_id}"], "play": False}))
    ...
    def play(self, token_manager):
        ...
        log.info(self.put(endpoint="me/player", headers=headers, json={"device_ids":[f"{self.device_id}"], "play": True}))
